...
const migrations = {
  ...
  patches: {
    ...
    16: player => {
      ...
      // the key-id pairs in speedrunMilestones
      const newArr = Array.repeat(0, 26);
      for (const entry of speedrunMilestones) {
        newArr[entry] = player.speedrun.records[speedrunMilestones[entry]];
      }
  ...
  fixChallengeIds(player) {
    ...
    if (wasFucked && player.challengeTimes) {
      player.challengeTimes = legacyIds.slice(1)
        .map(id => player.challengeTimes[id - 2]);
    }
  },
  ...
  convertAchivementsToNumbers(player) {
    ...
    for (const oldId of old) {
      const achByName = normalAchs.find(a => a.name === oldId);
      ...
      if (oldId.startsWith("r")) {
        if (normalAchs.find(a => a.id === newId) === undefined) {
  ...
  patch(saveData, maxVersion) {
    ...
    const player = deepmergeAll([Player, saveData]);
